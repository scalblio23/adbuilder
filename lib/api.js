...
function storeCreative(input) {
  ...
  if (input.previewUrl) meta.previewUrl = str(input.previewUrl, 2000).trim();
  var blob;
  if (input.data) {
...
function publicSettings(cfg) {
  var cred = cfg.auth === 'hmac' ? cfg.secret : cfg.key;
  return { hermesUrl: cfg.url, hermesKeyMasked: maskKey(cfg.key), webhookSecretMasked: cfg.secret ? cfg.secret.slice(0, 6) + '…' + cfg.secret.slice(-4) : '', webhookSecretSet: !!cfg.secret,
    hermesConfigured: !!(cfg.url && cred), source: cfg.source,
  ...
  if (req.method === 'PUT') {
    ...
    return docs.get('settings', 'hermes').then(function (existing) {
      ...
      var meta = {
        ...
        secret: body.webhookSecret ? str(body.webhookSecret, 500).trim() : (cur.secret || ''),
        auth: body.webhookSecret ? 'hmac' : (['bearer', 'x-api-key', 'both', 'hmac'].indexOf(body.auth) !== -1 ? body.auth : cur.auth),
...
// POST /api/settings/webhook-secret -> generate (shown once); DELETE -> clear. This is the WEBHOOK_SECRET to paste into Hermes.
function webhookSecret(req, res) {
  if (req.method !== 'POST' && req.method !== 'DELETE') return notAllowed(res, 'POST, DELETE');
  return docs.get('settings', 'hermes').then(function (existing) {
    var meta = Object.assign({}, hermes.DEFAULTS, existing || {});
    delete meta.id; delete meta.createdAt; delete meta.updatedAt;
    var fresh = req.method === 'POST' ? 'whsec_' + crypto.randomBytes(32).toString('hex') : '';
    meta.secret = fresh;
    if (req.method === 'POST') meta.auth = 'hmac';
    return docs.put('settings', 'hermes', meta).then(function () { return hermes.config(); }).then(function (cfg) {
      var view = publicSettings(cfg);
      if (fresh) view.secret = fresh;
      send(res, 200, view);
    });
  }).catch(function (e) { fail(res, e); });
}
...
var META_KINDS = { adAccounts: 'act', pixels: 'pixel', pages: 'page', campaigns: 'campaign' };
function normalizeMetaItem(kind, item) {
  if (!item || typeof item !== 'object') return null;
  var id = String(item.id || item.account_id || item.pixel_id || item.dataset_id || item.page_id || item.campaign_id || '').trim();
  ...
  if (kind === 'campaigns') {
    ...
    var acct = item.adAccountId || item.ad_account_id || item.account_id || (item.account && item.account.id) || '';
    if (acct) out.adAccountId = /^act_/.test(String(acct)) ? str(acct, 40) : 'act_' + str(acct, 40);
    if (item.adAccountName || item.ad_account_name || item.account_name) out.adAccountName = str(item.adAccountName || item.ad_account_name || item.account_name, 200);
    if (item.status || item.effective_status) out.status = str(item.status || item.effective_status, 40);
    if (item.objective) out.objective = str(item.objective, 80);
  }
  return out;
}
function storeMeta(body) {
  ...
  return Object.keys(META_KINDS).reduce(function (chain, kind) {
    ...
    if (kind === 'campaigns' && list && !Array.isArray(list) && Array.isArray(list.data)) list = list.data;
    ...
    if (kind !== 'campaigns') return chain.then(function () { return docs.put('meta', kind, { items: items, syncedAt: Date.now() }); });
    // Campaigns arrive per ad account: replace that account's campaigns, keep the other accounts' ones.
    return chain.then(function () { return docs.get('meta', 'campaigns'); }).then(function (row) {
      var touched = {}; items.forEach(function (i) { if (i.adAccountId) touched[i.adAccountId] = true; });
      var byId = {};
      ((row && row.items) || []).forEach(function (i) { if (!(i.adAccountId && touched[i.adAccountId])) byId[i.id] = i; });
      items.forEach(function (i) { byId[i.id] = Object.assign({}, byId[i.id] || {}, i, { syncedAt: Date.now() }); });
      var merged = Object.keys(byId).map(function (k) { return byId[k]; }).slice(-4000);
      return docs.put('meta', 'campaigns', { items: merged, syncedAt: Date.now() });
    });
...
// POST /api/meta/sync -> pull ad accounts, Pages and pixels straight from Meta with the stored token
function metaSync(req, res) {
  if (req.method !== 'POST') return notAllowed(res, 'POST');
  metaApi.config().then(function (c) {
    if (!c.set) throw metaApi.notConfigured();
    return metaApi.catalog(c.token);
  }).then(function (cat) {
    return storeMeta({ adAccounts: cat.adAccounts, pages: cat.pages, pixels: cat.pixels }).then(function (counts) { send(res, 200, { ok: true, counts: counts, warnings: cat.warnings }); });
  }).catch(function (e) { hermesFailure(res, e); });
}
...
function cleanSwipe(input, existing) {
  ...
  return {
    ...
    // Where the ad sat in the Ad Library results when Hermes saw it: 1 = top-left = best performer.
    libraryPosition: (function () { var v = input.libraryPosition != null ? input.libraryPosition : (input.library_position != null ? input.library_position : input.position); var n = Math.round(num(v, 0)); return n >= 1 ? n : (cur.libraryPosition || null); })(),
    positionSeenAt: (input.libraryPosition != null || input.library_position != null || input.position != null) ? now : (cur.positionSeenAt || null),
  ...
  if (req.method === 'GET') {
    return docs.list('swipes').then(function (rows) {
      rows.sort(function (a, b) {
        var pa = a.libraryPosition || 1e9, pb = b.libraryPosition || 1e9;
        return (pa - pb) || (b.ranking - a.ranking) || ((b.lastSeenAt || 0) - (a.lastSeenAt || 0));
      });
      send(res, 200, rows);
    }, function (e) { fail(res, e); });
  }
...
var inbound = {
  ...
  meta: guarded(function (req, res, id, body) {
    ...
    storeMeta(body).then(function (counts) {
      if (!Object.keys(counts).length) return send(res, 400, { error: 'Send at least one of adAccounts, pixels, pages, campaigns as arrays of { id, name }.' });
      return stats.catalogArrived().catch(function () {}).then(function () { send(res, 200, { ok: true, counts: counts }); });
    }, function (e) { fail(res, e); });
  }),
  ...
  // GET /api/v1/campaign-stats -> the tracked campaigns (what to pull); PUT { items: [{ campaignId, metrics, daily }] } -> Hermes pushes numbers in
  campaignStats: guarded(function (req, res, id, body) {
    if (req.method === 'GET') {
      return stats.listTracked().then(function (list) {
        send(res, 200, { campaigns: list.map(function (t) { return { id: t.id, name: t.name, adAccountId: t.adAccountId }; }), days: stats.DAYS, reportUrl: baseUrl(req) + '/api/v1/campaign-stats' });
      }, function (e) { fail(res, e); });
    }
    if (req.method !== 'PUT' && req.method !== 'POST') return notAllowed(res, 'GET, PUT, POST');
    stats.storeStats(body).then(function (r) { send(res, 200, { ok: true, stored: r.stored, skipped: r.skipped }); }, function (e) { hermesFailure(res, e); });
  }),
...
// ---- Tracked campaigns and their performance (Campaigns tab) ----
function hermesFailure(res, e) {
  if (e && e.code === 'META_AUTH') return send(res, 401, { error: e.message + ' The Meta token was rejected; paste a new one under Meta connection.' });
  if (e && e.code === 'META_RATE') return send(res, 429, { error: e.message + ' Meta is rate limiting; try again in a few minutes.' });
  if (e && (e.code === 'META_ERROR' || e.code === 'META_UNREACHABLE')) return send(res, 502, { error: e.message });
  if (e && e.code === 'NOT_CONFIGURED') return send(res, 501, { error: e.message });
  if (e && e.code === 'UNREACHABLE') return send(res, 502, { error: e.message });
  if (e && (e.code === 'NOTHING_TRACKED' || e.code === 'BAD_INPUT')) return send(res, 400, { error: e.message });
  if (e && e.code === 'COOLDOWN') return send(res, 429, { error: e.message });
  fail(res, e);
}
// GET /api/clients; PUT /api/clients/:id { name, accounts: ["act_…"] }; DELETE /api/clients/:id
function clients(req, res) {
  if (req.method !== 'GET') return notAllowed(res, 'GET');
  stats.listClients().then(function (l) { send(res, 200, l); }, function (e) { fail(res, e); });
}
function clientItem(req, res, id, body) {
  if (req.method === 'DELETE') return stats.removeClient(id).then(function (l) { send(res, 200, l); }, function (e) { fail(res, e); });
  if (req.method !== 'PUT' && req.method !== 'POST') return notAllowed(res, 'PUT, DELETE');
  stats.writeClient(id, body).then(function (l) { send(res, 200, l); }, function (e) { hermesFailure(res, e); });
}
// GET /api/bookings -> bookings logged per client; PUT /api/bookings/:accountId { metaAccountId, label, entries: [{ date, count }] }
function bookings(req, res) {
  if (req.method !== 'GET') return notAllowed(res, 'GET');
  stats.listBookings().then(function (list) { send(res, 200, list); }, function (e) { fail(res, e); });
}
function bookingsItem(req, res, id, body) {
  if (!validId(id)) return send(res, 404, { error: 'Account not found.' });   // an Ad Accounts row id or act_<meta id>
  if (req.method === 'GET') return stats.listBookings().then(function (list) { var b = list.filter(function (x) { return x.accountId === id; })[0]; b ? send(res, 200, b) : send(res, 404, { error: 'Account not found.' }); }, function (e) { fail(res, e); });
  if (req.method === 'DELETE') return stats.removeBookings(id).then(function () { send(res, 200, { ok: true }); }, function (e) { fail(res, e); });
  if (req.method !== 'PUT' && req.method !== 'POST') return notAllowed(res, 'GET, PUT, DELETE');
  stats.writeBookings(id, body).then(function (b) { b ? send(res, 200, b) : send(res, 404, { error: 'Account not found. Add the client in the Ad Accounts tab first.' }); }, function (e) { hermesFailure(res, e); });
}
// GET /api/tracked -> everything the Campaigns page shows; PUT { campaigns: [{ id, name, adAccountId }] } -> replace the tracked set
function tracked(req, res, body) {
  if (req.method === 'GET') return stats.overview().then(function (o) { send(res, 200, o); }, function (e) { fail(res, e); });
  if (req.method !== 'PUT') return notAllowed(res, 'GET, PUT');
  var list = body && (Array.isArray(body) ? body : body.campaigns);
  if (!Array.isArray(list)) return send(res, 400, { error: 'Send { campaigns: [{ id, name, adAccountId }] }.' });
  stats.setTracked(list).then(function () { return stats.overview(); }).then(function (o) { send(res, 200, o); }, function (e) { fail(res, e); });
}
// DELETE /api/tracked/:id -> stop tracking one campaign
function trackedItem(req, res, id, body) {
  if (req.method === 'PUT' || req.method === 'PATCH') {
    var patch = {}; if (body && body.resultOverride !== undefined) patch.resultOverride = body.resultOverride; if (body && body.client !== undefined) patch.client = body.client;
    return stats.patchTracked(id, patch).then(function (t) {
      if (!t) return send(res, 404, { error: 'Campaign is not tracked.' });
      return stats.overview().then(function (o) { send(res, 200, o); });
    }, function (e) { fail(res, e); });
  }
  if (req.method !== 'DELETE') return notAllowed(res, 'PUT, DELETE');
  stats.listTracked().then(function (list) {
    return stats.setTracked(list.filter(function (t) { return t.id !== id; }));
  }).then(function () { return stats.overview(); }).then(function (o) { send(res, 200, o); }, function (e) { fail(res, e); });
}
// POST /api/tracked/refresh -> ask Hermes (one webhook) for fresh numbers on the tracked campaigns only
// GET /api/ads/:id/download?kind=video|image -> the ad's creative file, streamed through the app with a
// download filename. Meta's CDN does not allow cross-origin downloads from the browser, and a video's
// file URL has to be fetched with the access token, so the server does both.
var MEDIA_HOSTS = /(^|\.)(facebook\.com|fbcdn\.net|fbsbx\.com|cdninstagram\.com)$/i;
function mediaAllowed(url) {
  var u; try { u = new URL(url); } catch (e) { return false; }
  if (u.protocol !== 'https:' && u.protocol !== 'http:') return false;
  return MEDIA_HOSTS.test(u.hostname) || (!!process.env.META_GRAPH_URL && /^(localhost|127\.0\.0\.1)$/.test(u.hostname));
}
function findAd(adId) {
  return docs.list('stats').then(function (rows) {
    var hit = null;
    rows.forEach(function (s) { (s.ads || []).forEach(function (a) { if (!hit && a.id === adId) hit = { ad: a, campaign: s }; }); });
    return hit;
  });
}
function adDownload(req, res, adId, query) {
  if (req.method !== 'GET') return notAllowed(res, 'GET');
  var kind = query && query.kind === 'image' ? 'image' : query && query.kind === 'video' ? 'video' : '';
  var stream = require('stream');
  findAd(adId).then(function (hit) {
    if (!hit) return send(res, 404, { error: 'That ad is not in the stored campaign data. Refresh the Campaigns tab and try again.' });
    var ad = hit.ad;
    if (!kind) kind = ad.videoId ? 'video' : 'image';
    var urlP;
    if (kind === 'video') {
      if (!ad.videoId) return send(res, 404, { error: 'This ad has no video.' });
      urlP = metaApi.config().then(function (c) { if (!c.set) throw metaApi.notConfigured(); return metaApi.videoSource(c.token, ad.videoId); });
    } else {
      if (!ad.imageUrl) return send(res, 404, { error: 'This ad has no full-size image. Refresh the Campaigns tab, then try again.' });
      urlP = Promise.resolve(ad.imageUrl);
    }
    return urlP.then(function (url) {
      if (!url) return send(res, 404, { error: 'Meta did not return a file for this ' + kind + '.' });
      if (!mediaAllowed(url)) return send(res, 502, { error: 'The file is not hosted by Meta, so it was not fetched.' });
      var controller = new AbortController();
      var timer = setTimeout(function () { controller.abort(); }, 55000);
      return fetch(url, { signal: controller.signal, redirect: 'follow' }).then(function (r) {
        if (!r.ok || !r.body) { clearTimeout(timer); return send(res, 502, { error: 'Meta returned HTTP ' + r.status + ' for the file.' }); }
        var type = (r.headers.get('content-type') || '').split(';')[0].trim() || (kind === 'video' ? 'video/mp4' : 'image/jpeg');
        var ext = { 'image/jpeg': 'jpg', 'image/png': 'png', 'image/gif': 'gif', 'image/webp': 'webp', 'video/mp4': 'mp4', 'video/quicktime': 'mov', 'video/webm': 'webm' }[type] || (kind === 'video' ? 'mp4' : 'jpg');
        var base = str(ad.name, 80).replace(/[^\w\s.-]+/g, ' ').replace(/\s+/g, ' ').trim() || 'ad';
        var name = base + ' ' + ad.id + '.' + ext;
        res.statusCode = 200;
        ...
        var len = r.headers.get('content-length'); if (len) res.setHeader('Content-Length', len);
        var body = stream.Readable.fromWeb(r.body);
        body.on('error', function () { clearTimeout(timer); res.destroy(); });
        body.on('end', function () { clearTimeout(timer); });
        body.pipe(res);
      }, function (err) {
        clearTimeout(timer);
        send(res, 502, { error: 'Could not fetch the file from Meta: ' + (err && err.name === 'AbortError' ? 'timed out' : err && err.message) });
      });
    });
  }).catch(function (e) { hermesFailure(res, e); });
}
function trackedRefresh(req, res) {
  if (req.method !== 'POST') return notAllowed(res, 'POST');
  hermes.config().then(function (cfg) { return stats.refresh('manual', cfg.publicBaseUrl || baseUrl(req)); })
    .then(function (record) { send(res, record.ok ? 200 : 502, { ok: record.ok, refresh: record, error: record.error || (record.ok ? undefined : 'Hermes returned ' + record.status) }); }, function (e) { hermesFailure(res, e); });
}
// GET/PUT /api/dashboard { metrics: [...] } -> which metrics the Campaigns tab shows (max 15)
function dashboard(req, res, body) {
  if (req.method === 'GET') return stats.readDashboard().then(function (d) { send(res, 200, d); }, function (e) { fail(res, e); });
  if (req.method !== 'PUT') return notAllowed(res, 'GET, PUT');
  stats.writeDashboard(body).then(function (d) { send(res, 200, d); }, function (e) { hermesFailure(res, e); });
}
// GET/PUT/DELETE /api/meta/token -> the Meta access token the app pulls stats with (checked against /me before saving)
function metaToken(req, res, body) {
  if (req.method === 'GET') return metaApi.config().then(function (c) { delete c.token; send(res, 200, c); }, function (e) { fail(res, e); });
  if (req.method === 'DELETE') {
    return docs.get('settings', 'meta').then(function (row) { return docs.put('settings', 'meta', Object.assign({}, row || {}, { token: '', user: '', accounts: null, checkedAt: null, error: '' })); })
      .then(function () { return metaApi.config(); }).then(function (c) { delete c.token; send(res, 200, c); }, function (e) { fail(res, e); });
  }
  if (req.method !== 'PUT' && req.method !== 'POST') return notAllowed(res, 'GET, PUT, DELETE');
  var token = str(body && body.token, 1000).trim();
  if (!token) return send(res, 400, { error: 'Paste the Meta access token.' });
  if (process.env.META_ACCESS_TOKEN) return send(res, 409, { error: 'The token is set through META_ACCESS_TOKEN on the host and cannot be changed here.' });
  var who = null;
  metaApi.whoAmI(token).then(function (me) { who = me; return metaApi.adAccounts(token); }).then(function (accounts) {
    return storeMeta({ adAccounts: accounts }).then(function () {
      return docs.put('settings', 'meta', { token: token, user: who && who.name || who && who.id || '', accounts: accounts.length, checkedAt: Date.now(), error: '' });
    });
  }).then(function () { return metaApi.config(); }).then(function (c) { delete c.token; send(res, 200, c); }, function (e) { hermesFailure(res, e); });
}
// POST /api/tracked/sync { accounts: ["act_…"] } -> ask Hermes for the campaign list of those accounts (or for the account list when none are known)
function trackedSync(req, res, body) {
  if (req.method !== 'POST') return notAllowed(res, 'POST');
  var ids = body && Array.isArray(body.accounts) ? body.accounts.map(function (a) { return str(a, 40); }) : [];
  hermes.config().then(function (cfg) { return stats.syncCatalog(ids, cfg.publicBaseUrl || baseUrl(req)); })
    .then(function (record) { send(res, record.ok ? 200 : 502, { ok: record.ok, sync: record, error: record.error || (record.ok ? undefined : 'Hermes returned ' + record.status) }); }, function (e) { hermesFailure(res, e); });
}
// GET /api/cron/stats -> Vercel cron (8:00 and 15:00 Adelaide). Needs CRON_SECRET; Vercel sends it as a Bearer token.
function cronStats(req, res) {
  var secret = process.env.CRON_SECRET;
  if (!secret) return send(res, 503, { error: 'CRON_SECRET is not set on the host, so scheduled refreshes are off.' });
  var given = String(req.headers.authorization || '').replace(/^Bearer\s+/i, '');
  var a = Buffer.from(given), b = Buffer.from(secret);
  if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) return send(res, 401, { error: 'Bad cron secret.' });
  hermes.config().then(function (cfg) { return stats.refresh('cron', cfg.publicBaseUrl || baseUrl(req)); })
    .then(function (record) { send(res, 200, { ok: record.ok, refresh: record, adelaide: stats.schedule().localNow }); },
      function (e) { if (e && e.code === 'NOTHING_TRACKED') return send(res, 200, { ok: true, skipped: e.message }); hermesFailure(res, e); });
}
...
module.exports = {
  adDownload: adDownload, clients: clients, clientItem: clientItem, bookings: bookings, bookingsItem: bookingsItem,
  ...
  apiKey: apiKey, inbound: inbound, meta: meta, metaSync: metaSync, hermesMeta: hermesMeta,
  swipes: swipes, swipe: swipe, webhookSecret: webhookSecret,
  tracked: tracked, trackedItem: trackedItem, trackedRefresh: trackedRefresh, trackedSync: trackedSync, cronStats: cronStats, metaToken: metaToken, storeMeta: storeMeta, dashboard: dashboard
};
