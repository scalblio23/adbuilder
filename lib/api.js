...
function creatives(req, res, body) {
  ...
  if (req.method === 'POST') {
    ...
    if (blob) meta.hash = crypto.createHash('sha256').update(Buffer.from(blob, 'base64')).digest('hex');
    else meta.hash = crypto.createHash('sha256').update(meta.url).digest('hex');
    meta.source = str(body.source, 40) || 'upload';
    // Same bytes already stored: hand back the existing record instead of a duplicate.
    return docs.list('creatives').then(function (rows) {
      var existing = rows.filter(function (r) { return r.hash === meta.hash; })[0];
      if (existing) return send(res, 200, withUrl(req, existing, true));
      return docs.put('creatives', docs.newId(), meta, blob).then(function (row) { send(res, 201, withUrl(req, row, false)); });
    }).catch(function (e) { fail(res, e); });
  }
  notAllowed(res, 'GET, POST');
}
