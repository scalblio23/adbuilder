...
function aiGenerate(req, res, body) {
  ...
  return docs.get('campaigns', body.campaignId).then(function (camp) {
    ...
    if (body.what === 'copy' || body.what === 'headlines') {
      return ai.generateList(body.what, brief, camp).then(function (items) { send(res, 200, { items: items }); });
    }
  ...
  }).catch(function (e) { send(res, e.code === 'NOT_CONFIGURED' ? 501 : 502, { error: e.message }); });
}
