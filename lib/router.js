...
var ROUTES = [
  ...
  [/^\/api\/ads\/([0-9]+)\/download\/?$/, function (req, res, id) { api.adDownload(req, res, id, query(req)); }],
  [/^\/api\/clients\/?$/, function (req, res) { api.clients(req, res); }],
  [/^\/api\/clients\/([^/]+)\/?$/, function (req, res, id, body) { api.clientItem(req, res, id, body); }],
  [/^\/api\/bookings\/?$/, function (req, res) { api.bookings(req, res); }],
  [/^\/api\/bookings\/([^/]+)\/?$/, function (req, res, id, body) { api.bookingsItem(req, res, id, body); }],
  ...
  [/^\/api\/meta\/sync\/?$/, function (req, res) { api.metaSync(req, res); }],
