...
function item(req, res, id, body) {
  ...
  if (req.method === 'PUT') {
    // Partial update: fields left out of the body keep their current values.
    return store.get(id).then(function (existing) {
      if (!existing) return send(res, 404, { error: 'Account not found.' });
      var merged = Object.assign({}, existing, body && typeof body === 'object' ? body : {});
      var data = store.clean(merged);
      if (!data) return send(res, 400, { error: 'Client name is required.' });
