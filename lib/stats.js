...
function cleanTracked(input) {
  ...
  return {
    ...
    resultOverride: str(input.resultOverride, 40).trim().toLowerCase().replace(/[^a-z_]/g, '')
  };
}
...
function setResultOverride(id, key) {
  return docs.get('tracked', id).then(function (t) {
    if (!t) return null;
    return docs.put('tracked', id, Object.assign({}, t, { resultOverride: str(key, 40).trim().toLowerCase().replace(/[^a-z_]/g, '') }));
  });
}
...
var METRICS = ['spend', 'impressions', 'reach', 'clicksAll', 'linkClicks', 'results', 'revenue', 'purchases', 'leads', 'newLeads', 'calls', 'refunds', 'refundAmount'];
var ALIASES = {
...
function cleanDaily(list) {
  ...
  list.forEach(function (d) {
    ...
    if (d.conv && typeof d.conv === 'object') {
      var conv = {}; Object.keys(d.conv).slice(0, 30).forEach(function (k) { if (/^[a-z_]{1,40}$/.test(k)) { var v = num(d.conv[k]); if (v) conv[k] = v; } });
...
function cleanStatsItem(item) {
  ...
  return {
    ...
    resultKey: str(item.resultKey || item.result_key, 40).trim().toLowerCase().replace(/[^a-z_]/g, ''),
    optimisation: (Array.isArray(item.optimisation) ? item.optimisation : []).slice(0, 50).map(function (o) { return { id: str(o && o.id, 40), name: str(o && o.name, 200), goal: str(o && o.goal, 60), event: str(o && o.event, 60) }; }),
...
function overview() {
  return Promise.all([listTracked(), docs.list('stats'), docs.get('settings', 'stats'), docs.get('meta', 'campaigns'), hermes.config(), knownAccounts(), docs.get('settings', 'apikey'), meta.config()]).then(function (r) {
    ...
    return {
      ...
      tracked: r[0].map(function (t) { var s = stats[t.id]; return Object.assign({}, t, { stats: s ? { metrics: s.metrics, daily: s.daily, ads: s.ads || [], resultType: s.resultType || '', resultKey: s.resultKey || '', optimisation: s.optimisation || [], objective: s.objective || '', currency: s.currency, period: s.period, syncedAt: s.syncedAt } : null }); }),
