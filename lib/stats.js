...
// A client is a label campaigns are attributed to (default: their ad account's name); campaigns from
// several ad accounts can share one. clientSlug turns the name into the key bookings are stored under.
function clientNameOf(t) { return (t && (t.client || t.adAccountName || t.adAccountId)) || 'No client'; }
function clientSlug(name) { return 'c-' + (String(name || '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 50) || 'client'); }
// Per-campaign choice of what counts as a result ("" = follow the ad sets' optimisation).
function setResultOverride(id, key) { return patchTracked(id, { resultOverride: key }); }
// Per-campaign settings: resultOverride ("" = follow the ad sets) and client ("" = the ad account's name).
function patchTracked(id, patch) {
  patch = patch || {};
  return docs.get('tracked', id).then(function (t) {
    if (!t) return null;
    var next = Object.assign({}, t);
    if (patch.resultOverride !== undefined) next.resultOverride = str(patch.resultOverride, 40).trim().toLowerCase().replace(/[^a-z0-9_]/g, '');
    if (patch.client !== undefined) next.client = str(patch.client, 120).trim();
    return docs.put('tracked', id, next);
  });
}
...
var MAX_METRICS = 15;
var DEFAULT_METRICS = ['spend', 'results', 'costPerResult', 'linkClicks', 'optinRate', 'leadToBooking', 'impressions', 'reach', 'cpm', 'cplc', 'ctrAll', 'linkCtr', 'frequency'];
// Always on the dashboard, even one saved before they existed (added at the end; trailing others give way if the 15 are full).
var ALWAYS_METRICS = ['linkClicks', 'optinRate', 'leadToBooking'];
function withAlways(list) {
  var out = list.slice();
  ALWAYS_METRICS.forEach(function (k) { if (out.indexOf(k) === -1) out.push(k); });
  while (out.length > MAX_METRICS) { var i = out.length - 1; while (i >= 0 && ALWAYS_METRICS.indexOf(out[i]) !== -1) i--; if (i < 0) break; out.splice(i, 1); }
  return out;
}
function readDashboard() {
  return docs.get('settings', 'dashboard').then(function (row) {
    ...
    return { metrics: withAlways(list.length ? list : DEFAULT_METRICS), max: MAX_METRICS, updatedAt: row ? row.updatedAt || null : null };
  });
}
...
// ---- Clients: named groups that own whole Meta ad accounts (docs "clients"/<slug>: { name, accounts }) ----
// A campaign counts under its own client override, else the client that owns its ad account, else the
// ad account's name. Each ad account belongs to at most one client.
function listClients() {
  return docs.list('clients').then(function (rows) {
    return rows.map(function (c) { return { id: c.id, name: c.name || c.id, accounts: Array.isArray(c.accounts) ? c.accounts : [], createdAt: c.createdAt || null }; })
      .sort(function (a, b) { return a.name.localeCompare(b.name); });
  });
}
function writeClient(id, body) {
  body = body || {};
  if (!/^c-[a-z0-9-]{1,60}$/.test(id)) return Promise.reject(Object.assign(new Error('Bad client id.'), { code: 'BAD_INPUT' }));
  return docs.get('clients', id).then(function (existing) {
    var next = Object.assign({ name: '', accounts: [], createdAt: Date.now() }, existing || {});
    if (body.name !== undefined) next.name = str(body.name, 120).trim();
    if (!next.name) throw Object.assign(new Error('The client needs a name.'), { code: 'BAD_INPUT' });
    if (body.accounts !== undefined) {
      if (!Array.isArray(body.accounts)) throw Object.assign(new Error('accounts must be a list of act_… ids.'), { code: 'BAD_INPUT' });
      var seen = {};
      next.accounts = body.accounts.map(function (a) { a = str(a, 40).trim(); if (a && !/^act_/.test(a)) a = 'act_' + a; return a; }).filter(function (a) { if (!/^act_[0-9]{3,30}$/.test(a) || seen[a]) return false; seen[a] = true; return true; }).slice(0, 200);
    }
    next.updatedAt = Date.now();
    // one owner per ad account: take them away from any other client
    return docs.list('clients').then(function (rows) {
      return rows.reduce(function (chain, c) {
        if (c.id === id) return chain;
        var kept = (c.accounts || []).filter(function (a) { return next.accounts.indexOf(a) === -1; });
        if (kept.length === (c.accounts || []).length) return chain;
        return chain.then(function () { return docs.put('clients', c.id, Object.assign({}, c, { accounts: kept })); });
      }, Promise.resolve());
    }).then(function () { return docs.put('clients', id, next); }).then(listClients);
  });
}
function removeClient(id) { return docs.remove('clients', id).then(listClients); }
// ad account id -> owning client id
function accountOwners(clients) { var o = {}; clients.forEach(function (c) { c.accounts.forEach(function (a) { if (!o[a]) o[a] = c.id; }); }); return o; }
...
// Bookings docs are keyed by an Ad Accounts row id (client) or directly by a Meta ad account id (act_…)
// when logged from the Campaigns tab; either way each resolves to the Meta ad account it counts for.
// Bookings docs are keyed by a client slug (c-…, logged from the Campaigns tab), by an Ad Accounts row id,
// or by a Meta ad account id (act_…, older). Each resolves to the client (clientKey) it counts for: for the
// older kinds that is the default client of the Meta ad account they name.
function listBookings() {
  var accounts = require('./store');
  return Promise.all([docs.list('bookings'), accounts.list(), listTracked(), listClients()]).then(function (r) {
    var rows = {}; r[0].forEach(function (b) { rows[b.id] = b; });
    var owners = accountOwners(r[3]);
    var acctClient = {}; r[2].forEach(function (t) { if (t.adAccountId && !acctClient[t.adAccountId]) acctClient[t.adAccountId] = owners[t.adAccountId] || clientSlug(t.adAccountName || t.adAccountId); });
    Object.keys(owners).forEach(function (a) { acctClient[a] = owners[a]; });
    var out = [], used = {};
    function total(entries) { return entries.reduce(function (n, e) { return n + (e.count || 0); }, 0); }
    (r[1] || []).forEach(function (a) {
      var b = rows[a.id] || {}; used[a.id] = true;
      var m = /[?&]act=(\d+)/.exec(String(a.link || '')) || /act_(\d+)/.exec(String(a.link || ''));
      var entries = Array.isArray(b.entries) ? b.entries : [];
      var acct = b.metaAccountId || (m ? 'act_' + m[1] : '');
      out.push({ accountId: a.id, client: a.client || '', company: a.company || '', metaAccountId: acct, clientKey: b.clientKey || (acct ? acctClient[acct] || '' : ''), linkedFromLink: !b.metaAccountId && !!m, label: b.label || 'Bookings', entries: entries, total: total(entries), updatedAt: b.updatedAt || null });
    });
    r[0].forEach(function (b) {
      if (used[b.id]) return;
      var entries = Array.isArray(b.entries) ? b.entries : [];
      if (/^c-[a-z0-9-]{1,60}$/.test(b.id)) out.push({ accountId: b.id, client: b.client || b.id, company: '', metaAccountId: '', clientKey: b.id, linkedFromLink: false, label: b.label || 'Bookings', entries: entries, total: total(entries), updatedAt: b.updatedAt || null });
      else if (/^act_[0-9]{3,30}$/.test(b.id)) out.push({ accountId: b.id, client: b.client || '', company: '', metaAccountId: b.id, clientKey: acctClient[b.id] || '', linkedFromLink: false, label: b.label || 'Bookings', entries: entries, total: total(entries), updatedAt: b.updatedAt || null });
    });
    return out;
  });
}
function writeBookings(accountId, body) {
  var accounts = require('./store');
  var direct = /^act_[0-9]{3,30}$/.test(accountId), byClient = /^c-[a-z0-9-]{1,60}$/.test(accountId);
  return (direct || byClient ? Promise.resolve({ id: accountId }) : accounts.get(accountId)).then(function (a) {
    if (!a) return null;
    return docs.get('bookings', accountId).then(function (existing) {
      var next = cleanBookings(body, existing); next.updatedAt = Date.now();
      if (direct) next.metaAccountId = accountId;
      if (byClient) { next.clientKey = accountId; next.metaAccountId = ''; }
      if ((direct || byClient) && body && body.client !== undefined) next.client = str(body.client, 200).trim();
...
function overview() {
  return Promise.all([listTracked(), docs.list('stats'), docs.get('settings', 'stats'), docs.get('meta', 'campaigns'), hermes.config(), knownAccounts(), docs.get('settings', 'apikey'), meta.config(), readDashboard(), listBookings(), listClients()]).then(function (r) {
...
// Pull straight from Meta. Stalest campaigns first, a few at a time, each stored the moment it lands, and
// the run stops starting new ones once the time budget is used up (a serverless function has a hard limit),
// reporting what is left so the caller can call again. Nothing is lost if the run dies part way.
var REFRESH_BUDGET_MS = Number(process.env.REFRESH_BUDGET_MS) || 40000;
var REFRESH_PARALLEL = Number(process.env.REFRESH_PARALLEL) || 3;
function directRefresh(source, tracked, settings, token) {
  var since = today(-(DAYS - 1)), until = today(0);
  var started = Date.now(), stored = [], errors = [], queue, remaining = [], attempted = 0;
  return docs.list('stats').then(function (rows) {
    var syncedAt = {}; rows.forEach(function (r) { syncedAt[r.id] = r.syncedAt || 0; });
    queue = tracked.slice().sort(function (a, b) { return (syncedAt[a.id] || 0) - (syncedAt[b.id] || 0); });
    function worker() {
      if (!queue.length) return Promise.resolve();
      if (attempted > 0 && Date.now() - started > REFRESH_BUDGET_MS) { remaining = remaining.concat(queue.splice(0)); return Promise.resolve(); }   // always make progress: at least one per run
      var t = queue.shift(); attempted++;
      return meta.campaignStats(token, t, since, until)
        .then(function (item) { return storeStats({ items: [item] }).then(function (r) { if (r.stored.length) stored.push(t.id); }); },
              function (err) { errors.push({ id: t.id, name: t.name, error: err.message, code: err.code }); })
        .then(worker);
    }
    var workers = []; for (var i = 0; i < Math.max(1, REFRESH_PARALLEL); i++) workers.push(worker());
    return Promise.all(workers);
  }).then(function () {
    if (!stored.length && !remaining.length) { var e = new Error(errors[0] ? errors[0].error : 'Meta returned nothing.'); e.code = errors[0] && errors[0].code === 'META_AUTH' ? 'META_AUTH' : 'META_ERROR'; throw e; }
    return docs.get('settings', 'stats').then(function (fresh) {
      fresh = fresh || {};
      var record = { at: Date.now(), source: source, direct: true, campaigns: tracked.length, stored: stored.length, errors: errors, remaining: remaining.length, remainingNames: remaining.slice(0, 10).map(function (t) { return t.name; }), tookMs: Date.now() - started, status: 200, ok: true };
...
function refresh(source, baseUrl) {
  return Promise.all([listTracked(), hermes.config(), docs.get('settings', 'stats'), meta.config()]).then(function (r) {
    ...
    var unfinished = settings.lastRefresh && settings.lastRefresh.remaining > 0;
    if (source === 'manual' && !unfinished && Date.now() - last < (mc.set ? 15000 : REFRESH_COOLDOWN_MS)) {
