...
function listBookings() {
  ...
  return Promise.all([docs.list('bookings'), accounts.list()]).then(function (r) {
    ...
    (r[1] || []).forEach(function (a) {
      ...
      out.push({ accountId: a.id, client: a.client || '', company: a.company || '', metaAccountId: b.metaAccountId || (m ? 'act_' + m[1] : ''), linkedFromLink: !b.metaAccountId && !!m, label: b.label || 'Bookings', entries: entries, total: entries.reduce(function (n, e) { return n + (e.count || 0); }, 0), updatedAt: b.updatedAt || null });
    });
    r[0].forEach(function (b) {
      if (used[b.id] || !/^act_[0-9]{3,30}$/.test(b.id)) return;
      var entries = Array.isArray(b.entries) ? b.entries : [];
      out.push({ accountId: b.id, client: b.client || '', company: '', metaAccountId: b.id, linkedFromLink: false, label: b.label || 'Bookings', entries: entries, total: entries.reduce(function (n, e) { return n + (e.count || 0); }, 0), updatedAt: b.updatedAt || null });
    });
    return out;
  });
}
function writeBookings(accountId, body) {
  var accounts = require('./store');
  var direct = /^act_[0-9]{3,30}$/.test(accountId);
  return (direct ? Promise.resolve({ id: accountId }) : accounts.get(accountId)).then(function (a) {
    if (!a) return null;
    return docs.get('bookings', accountId).then(function (existing) {
      var next = cleanBookings(body, existing); next.updatedAt = Date.now();
      if (direct) { next.metaAccountId = accountId; if (body && body.client !== undefined) next.client = str(body.client, 200).trim(); }
