...
function overview() {
  return Promise.all([listTracked(), docs.list('stats'), docs.get('settings', 'stats'), docs.get('meta', 'campaigns'), hermes.config(), knownAccounts(), docs.get('settings', 'apikey'), meta.config()]).then(function (r) {
    ...
    return {
      ...
      tracked: r[0].map(function (t) { var s = stats[t.id]; return Object.assign({}, t, { stats: s ? { metrics: s.metrics, daily: s.daily, ads: s.ads || [], resultType: s.resultType || '', objective: s.objective || '', currency: s.currency, period: s.period, syncedAt: s.syncedAt } : null }); }),
