...
function overview() {
  return Promise.all([listTracked(), docs.list('stats'), docs.get('settings', 'stats'), docs.get('meta', 'campaigns'), hermes.config()]).then(function (r) {
...
function refresh(source, baseUrl) {
  return Promise.all([listTracked(), hermes.config(), docs.get('settings', 'stats')]).then(function (r) {
    ...
    return hermes.send(cfg, body, 'adbuilder.stats.refresh').then(function (reply) {
      var record = { at: Date.now(), source: source, campaigns: tracked.length, status: reply.status, ok: reply.ok, reply: reply.body };
...
module.exports = { listTracked: listTracked, setTracked: setTracked, storeStats: storeStats, overview: overview, refresh: refresh, schedule: schedule, refreshPrompt: refreshPrompt, cleanStatsItem: cleanStatsItem, DAYS: DAYS };
