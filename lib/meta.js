...
// The catalog the Ad Builder dropdowns need: ad accounts, the Pages the token can promote, and each
// account's pixels. Every source is optional (a System User token may lack some permissions), so
// a failing edge is skipped and reported rather than failing the whole sync.
function catalog(token) {
  var out = { adAccounts: [], pages: [], pixels: [], warnings: [] };
  var pages = {};
  function addPages(list) { (list || []).forEach(function (p) { if (p && p.id && !pages[p.id]) pages[p.id] = { id: String(p.id), name: p.name || String(p.id), category: p.category || '' }; }); }
  function attempt(label, fn) { return fn().catch(function (e) { out.warnings.push(label + ': ' + (e && e.message ? e.message.replace(/^Meta API: /, '') : 'failed')); }); }
  return adAccounts(token).then(function (accts) {
    out.adAccounts = accts;
    return attempt('pages', function () { return graphAll(token, '/me/accounts', { fields: 'id,name,category' }).then(addPages); });
  }).then(function () {
    return attempt('business pages', function () {
      return graphAll(token, '/me/businesses', { fields: 'id,name' }).then(function (bizs) {
        return bizs.slice(0, 20).reduce(function (chain, b) {
          return chain.then(function () { return pause(CALL_GAP_MS); })
            .then(function () { return graphAll(token, '/' + b.id + '/owned_pages', { fields: 'id,name,category' }).then(addPages, function () {}); })
            .then(function () { return graphAll(token, '/' + b.id + '/client_pages', { fields: 'id,name,category' }).then(addPages, function () {}); });
        }, Promise.resolve());
      });
    });
  }).then(function () {
    return out.adAccounts.slice(0, 50).reduce(function (chain, a) {
      return chain.then(function () { return pause(CALL_GAP_MS); })
        .then(function () { return graphAll(token, '/' + a.id + '/promote_pages', { fields: 'id,name,category' }).then(addPages, function () {}); })
        .then(function () { return attempt('pixels for ' + a.name, function () { return graphAll(token, '/' + a.id + '/adspixels', { fields: 'id,name' }).then(function (list) { list.forEach(function (px) { out.pixels.push({ id: String(px.id), name: px.name || String(px.id), adAccountId: a.id }); }); }); }); });
    }, Promise.resolve());
  }).then(function () {
    out.pages = Object.keys(pages).map(function (k) { return pages[k]; });
    var seen = {}; out.pixels = out.pixels.filter(function (px) { if (seen[px.id]) return false; seen[px.id] = true; return true; });
    return out;
  });
}
function campaigns(token, accounts) {
  ...
  return accounts.reduce(function (chain, acct) {
    return chain.then(function () {
      return graphAll(token, '/' + acct.id + '/campaigns', { fields: 'id,name,status,effective_status,objective,updated_time', effective_status: ['ACTIVE', 'PAUSED', 'IN_PROCESS', 'WITH_ISSUES'] }).then(function (list) {
...
// The downloadable media behind an ad: the full-size image (image, carousel, video poster or asset feed)
// and the id of its video, if it has one. A video's file URL is fetched on demand (videoSource) because it expires.
function mediaOf(creative) {
  var out = { imageUrl: '', videoId: '' };
  if (!creative) return out;
  var spec = creative.object_story_spec || {}, feed = creative.asset_feed_spec || {};
  var images = [creative.image_url, spec.link_data && spec.link_data.picture, spec.video_data && spec.video_data.image_url, spec.photo_data && spec.photo_data.url];
  ((spec.link_data && spec.link_data.child_attachments) || []).forEach(function (c) { images.push(c.picture); });
  (feed.images || []).forEach(function (i) { images.push(i.url); });
  out.imageUrl = (images.filter(function (u) { return typeof u === 'string' && /^https?:\/\//i.test(u); })[0] || '').slice(0, 1000);
  var videos = [creative.video_id, spec.video_data && spec.video_data.video_id];
  ((spec.link_data && spec.link_data.child_attachments) || []).forEach(function (c) { videos.push(c.video_id); });
  (feed.videos || []).forEach(function (v) { videos.push(v.video_id); });
  out.videoId = String(videos.filter(function (v) { return v != null && /^[0-9]{5,30}$/.test(String(v)); })[0] || '');
  return out;
}
// The (temporary) file URL of an ad video.
function videoSource(token, videoId) {
  return graph(token, '/' + videoId, { fields: 'id,source,title' }).then(function (v) { return v.source || ''; });
}
...
function campaignStats(token, campaign, since, until) {
  ...
  return graph(token, '/' + campaign.id, { fields: 'id,name,status,effective_status,objective' }).then(function (c) {
    ...
    return graphAll(token, '/' + campaign.id + '/ads', { fields: 'id,name,effective_status,status,adset{id,name},creative{thumbnail_url,image_url,video_id,object_story_spec,asset_feed_spec},preview_shareable_link' });
  }).then(function (ads) {
    var byId = {};
    ads.forEach(function (a) {
      byId[a.id] = { id: a.id, name: a.name || 'Ad ' + a.id, status: a.effective_status || a.status || '', adSetId: a.adset && a.adset.id || '', adSetName: a.adset && a.adset.name || '', thumbnailUrl: a.creative && a.creative.thumbnail_url || '', imageUrl: mediaOf(a.creative).imageUrl, videoId: mediaOf(a.creative).videoId, linkUrl: destinationOf(a.creative), previewUrl: a.preview_shareable_link || '', daily: [] };
    });
...
module.exports = { config: config, notConfigured: notConfigured, whoAmI: whoAmI, adAccounts: adAccounts, catalog: catalog, campaigns: campaigns, campaignStats: campaignStats, videoSource: videoSource, pause: pause, LABELS: LABELS, labelFor: labelFor };
