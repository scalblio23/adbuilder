...
function dayRow(row, key) {
  ...
  (row.actions || []).forEach(function (a) { var m = /^offsite_conversion\.custom\.(\d+)$/.exec(a.action_type || ''); if (m && Number(a.value)) conv['custom_' + m[1]] = Math.max(conv['custom_' + m[1]] || 0, Number(a.value)); });
  var raw = {}; (row.actions || []).forEach(function (a) { if (Number(a.value)) raw[String(a.action_type).slice(0, 60)] = Number(a.value); });
  var out = {
    date: row.date_start,
    ...
    postEngagement: actionMax(row.actions, ['post_engagement']),
    pageEngagement: actionMax(row.actions, ['page_engagement']),
    reactions: actionMax(row.actions, ['post_reaction']),
    comments: actionMax(row.actions, ['comment']),
    shares: actionMax(row.actions, ['post']),
    saves: actionMax(row.actions, ['onsite_conversion.post_save']),
    pageLikes: actionMax(row.actions, ['like']),
    videoPlays: listValue(row.video_play_actions),
    videoViews3s: actionMax(row.actions, ['video_view']),
...
var INSIGHT_FIELDS = 'date_start,spend,impressions,reach,clicks,inline_link_clicks,unique_clicks,unique_inline_link_clicks,outbound_clicks,actions,action_values,video_play_actions,video_thruplay_watched_actions,video_p25_watched_actions,video_p50_watched_actions,video_p75_watched_actions,video_p100_watched_actions,social_spend,account_currency,objective';
