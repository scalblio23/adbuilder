...
function campaignStats(token, campaign, since, until) {
  ...
  return graph(token, '/' + campaign.id, { fields: 'id,name,status,effective_status,objective' }).then(function (c) {
    ...
    return graphAll(token, '/' + campaign.id + '/ads', { fields: 'id,name,effective_status,status,adset{id,name},creative{thumbnail_url,image_url},preview_shareable_link' });
  }).then(function (ads) {
    var byId = {};
    ads.forEach(function (a) {
      byId[a.id] = { id: a.id, name: a.name || 'Ad ' + a.id, status: a.effective_status || a.status || '', adSetId: a.adset && a.adset.id || '', adSetName: a.adset && a.adset.name || '', thumbnailUrl: a.creative && a.creative.thumbnail_url || '', imageUrl: a.creative && a.creative.image_url || '', previewUrl: a.preview_shareable_link || '', daily: [] };
    });
