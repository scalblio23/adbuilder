...
// Rules 1-4 (see AI_RULES.md): headlines derive from filled primary text, copy from filled
// headlines; either falls back to the brief; with no source at all, ask for input.
function generateList(kind, brief, campaign) {
  var d = campaign.data || {};
  var copy = (d.copy || []).map(function (c) { return String(c || '').trim(); }).filter(Boolean);
  var headlines = (d.headlines || []).map(function (c) { return String(c || '').trim(); }).filter(Boolean);
  var source, sourceLabel;
  if (kind === 'headlines' && copy.length) { source = copy.map(function (c, i) { return (i + 1) + '. ' + c; }).join('\n'); sourceLabel = 'primary text'; }
  else if (kind === 'copy' && headlines.length) { source = headlines.map(function (c, i) { return (i + 1) + '. ' + c; }).join('\n'); sourceLabel = 'headlines'; }
  else if (brief) { source = brief; sourceLabel = 'brief'; }
  else {
    var e = new Error(kind === 'headlines'
      ? 'Write at least one primary text in step 1 (or a brief in "Let AI build") first. Headlines are derived from it.'
      : 'Write at least one headline in step 2 (or a brief in "Let AI build") first. Copy is derived from it.');
    e.code = 'NEEDS_INPUT'; throw e;
  }
  var ask = kind === 'headlines'
    ? 'Write 5 headlines under 40 characters. Each must match the angle and offer of the ' + sourceLabel + ' above and work as the bold line under one of those primary texts.'
    : 'Write 4 primary text variants, each 40-125 words, different angles. Each must deliver on the promise of the ' + sourceLabel + ' above.';
  var existing = kind === 'headlines' ? headlines : copy;
  var prompt = 'Campaign: ' + campaign.name + '\n' + (brief && sourceLabel !== 'brief' ? 'Brief: ' + brief + '\n' : '') +
    '\n' + sourceLabel.toUpperCase() + ':\n' + source + '\n\n' +
    (existing.length ? 'Already written (do not repeat these):\n' + existing.map(function (c) { return '- ' + c; }).join('\n') + '\n\n' : '') +
    ask + ' Return JSON: { "items": [string] }';
  return complete({ system: SYSTEM, prompt: prompt, maxTokens: 2000 }).then(parseJson).then(function (j) { return { items: (j.items || []).map(String).filter(Boolean), source: sourceLabel }; });
}
function test() {
