...
function headers(cfg, raw, eventType) {
  ...
  if (cfg.auth === 'hmac') {
    ...
    var sig = crypto.createHmac('sha256', secret).update(raw || '').digest('hex');
    h['X-Hub-Signature-256'] = 'sha256=' + sig;     // GitHub style
    h['X-Signature-256'] = 'sha256=' + sig;
    h['X-Webhook-Signature'] = 'sha256=' + sig;
    h['X-Hub-Signature'] = 'sha1=' + crypto.createHmac('sha1', secret).update(raw || '').digest('hex');
    h['X-Hermes-Timestamp'] = String(Date.now());
  }
  return h;
}
...
function test() {
  return config().then(function (cfg) {
    ...
    var body = method === 'GET' || method === 'HEAD' ? null : { event: 'adbuilder.test', prompt: 'Connection test from AdBuilder. Reply with OK.', source: 'adbuilder', test: true, sentAt: new Date().toISOString() };
