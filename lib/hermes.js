...
// How the key travels. "hmac" signs the exact request body with the webhook secret (sha256),
// which is what Hermes' webhook receiver (and GitHub-style receivers) verify:
//   X-Hub-Signature-256      sha256=<hmac(body)>               GitHub style (Hermes accepts it)
//   X-Webhook-Signature-V2   <hmac("<timestamp>.<body>")>      Hermes "generic V2", with X-Webhook-Timestamp
// Every request also carries a unique X-Request-ID / X-GitHub-Delivery so a retry never starts a second run.
function headers(cfg, raw, eventType) {
  var h = { 'Accept': 'application/json, text/plain, */*', 'User-Agent': 'AdBuilder/1.0', 'X-Source': 'adbuilder' };
  if (raw != null) h['Content-Type'] = 'application/json';
  if (eventType) { h['X-Event-Type'] = eventType; h['X-GitHub-Event'] = eventType; }
  var deliveryId = crypto.randomUUID();
  h['X-Request-ID'] = deliveryId; h['X-GitHub-Delivery'] = deliveryId;
  if (cfg.auth === 'bearer' || cfg.auth === 'both') h['Authorization'] = 'Bearer ' + cfg.key;
  if (cfg.auth === 'x-api-key' || cfg.auth === 'both') h['X-API-Key'] = cfg.key;
  if (cfg.auth === 'hmac') {
    var secret = credential(cfg);
    var body = raw || '';
    var ts = String(Math.floor(Date.now() / 1000));
    var sig = crypto.createHmac('sha256', secret).update(body).digest('hex');
    h['X-Hub-Signature-256'] = 'sha256=' + sig;
    h['X-Hub-Signature'] = 'sha1=' + crypto.createHmac('sha1', secret).update(body).digest('hex');
    h['X-Webhook-Timestamp'] = ts;
    h['X-Webhook-Signature-V2'] = crypto.createHmac('sha256', secret).update(ts + '.' + body).digest('hex');
  }
  return h;
}
...
function call(cfg, method, path, body, eventType) {
  ...
  var raw = body ? JSON.stringify(body) : null;
  return fetch(cfg.url + (path || ''), { method: method, headers: headers(cfg, raw, eventType), body: raw || undefined, signal: controller.signal })
    .then(function (res) {
...
function test() {
  return config().then(function (cfg) {
    ...
    var body = method === 'GET' || method === 'HEAD' ? null : { event: 'adbuilder.test', event_type: 'adbuilder.test', prompt: 'Connection test from AdBuilder. Reply with OK.', source: 'adbuilder', test: true, sentAt: new Date().toISOString() };
    return call(cfg, method, cfg.testPath, body, 'adbuilder.test').then(function (r) { return Object.assign({ request: method + ' ' + cfg.url + cfg.testPath, signed: cfg.auth === 'hmac' }, r); });
  });
}
...
// Any other one-off webhook to Hermes (same route as Launch): returns { status, ok, body, ms }.
function send(cfg, body, eventType) { return call(cfg, 'POST', cfg.launchPath, body, eventType); }
...
module.exports = { config: config, test: test, launch: launch, send: send, pullAccounts: pullAccounts, pullMeta: pullMeta, payload: payload, buildPrompt: buildPrompt, DEFAULTS: DEFAULTS };
