...
var STATUS_VALUES = ['', 'green', 'amber', 'red', 'notlive'];
...
function postgresStore() {
  ...
  function q(text, params) {
    return db.ensure('ad_accounts', setup).then(function () { return db.query(text, params); });
  }
  function setup() {
    return db.query(
      ...
      return db.query("ALTER TABLE ad_accounts ADD COLUMN IF NOT EXISTS rules TEXT NOT NULL DEFAULT ''");
    }).then(function () {
      return STATUS_FIELDS.reduce(function (chain, f) {
        return chain.then(function () { return db.query('ALTER TABLE ad_accounts ADD COLUMN IF NOT EXISTS ' + f + " TEXT NOT NULL DEFAULT ''"); });
      }, Promise.resolve());
    }).then(function () {
      return db.query("SELECT 1 FROM information_schema.columns WHERE table_name='ad_accounts' AND column_name='urls'").then(function (r) {
        if (!r.rowCount) return;
        return db.query("UPDATE ad_accounts SET rules = urls WHERE rules = '' AND urls <> ''")
          .then(function () { return db.query('ALTER TABLE ad_accounts DROP COLUMN urls'); });
      });
    });
  }
...
var store = require('./db').enabled ? postgresStore() : fileStore();
store.clean = clean;
