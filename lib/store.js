...
function clean(input) {
  ...
  return { client: client, company: company, link: link, rules: rules };
}
...
function postgresStore() {
  ...
  function row(r) {
    return { id: r.id, client: r.client, company: r.company, link: r.link, rules: r.rules || '', createdAt: Number(r.created_at), updatedAt: Number(r.updated_at) };
  }
  ...
  return {
    ...
    create: function (data) {
      var now = Date.now();
      var id = newId();
      return q('INSERT INTO ad_accounts (id, client, company, link, rules, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
        [id, data.client, data.company, data.link, data.rules, now, now]).then(function (r) { return row(r.rows[0]); });
    },
    update: function (id, data) {
      return q('UPDATE ad_accounts SET client=$2, company=$3, link=$4, rules=$5, updated_at=$6 WHERE id=$1 RETURNING *',
        [id, data.client, data.company, data.link, data.rules, Date.now()]).then(function (r) { return r.rows[0] ? row(r.rows[0]) : null; });
    },
    remove: function (id) {
...
function fileStore() {
  ...
  return {
    ...
    create: function (data) {
      var rows = read();
      var now = Date.now();
      var item = { id: newId(), client: data.client, company: data.company, link: data.link, rules: data.rules, createdAt: now, updatedAt: now };
    ...
    update: function (id, data) {
      ...
      rows[i] = { id: id, client: data.client, company: data.company, link: data.link, rules: data.rules, createdAt: rows[i].createdAt, updatedAt: Date.now() };
      write(rows);
